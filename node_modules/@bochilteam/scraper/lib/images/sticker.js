...
async function stickerTelegram(query, page) {
    const { data } = await axios_1.default.get(`https://combot.org/telegram/stickers?q=${encodeURI(query)}&page=${page || 1}`);
...
async function stickerLine(query) {
    const { data } = await axios_1.default.get(`https://store.line.me/api/search/sticker?query=${query}&offset=0&limit=36&type=ALL&includeFacets=true`);
